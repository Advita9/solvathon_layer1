...
def _generate_summary(history: List[Dict], ollama_url: str = "http://localhost:11434") -> str:
    """
...
class RedisContextManager:
    ...
    def _async_summarize(self, session_id: str, history_to_summarize: List[Dict]):
        ...
        try:
            ...
            if session:
                ...
                print(f"[INFO] Async summarization complete for {session_id}: {summary[:50]}...")
        except Exception as e:
    ...
    def add_turn(self, session_id: str, role: str, content: str) -> bool:
        ...
        if history_len >= max_messages:
            if session.get("summary"):
                ...
                print(f"[INFO] Trimmed with summary for {session_id}, keeping {keep_count} messages")
            else:
    ...
    def trigger_summarization_during_stt(self, session_id: str) -> bool:
        ...
        if history_len >= max_messages - 4 or session.get("needs_summarization"):
            ...
            summarize_count = min(history_len // 2, 6)  # Summarize first half, max 6 messages
    ...
    def is_summarization_needed(self, session_id: str) -> bool:
        ...
        return history_len >= max_messages - 4 or session.get("needs_summarization", False)
