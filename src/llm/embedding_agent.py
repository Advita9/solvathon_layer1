...
def process_with_context(
    ...
    # Inject metadata into system prompt if available
    if metadata:
        try:
            defaults = {
    ...
    # Create/Get session logic (Same as before)
    ...
    # Step 3: Add user turn
    try:
        context_manager.add_turn(session_id, "user", prompt)
    except Exception as e:
        print(f"[WARN] Redis context add_turn failed: {e}")
    
    # Step 4: Get context
    try:
        messages = context_manager.get_context_for_llm(session_id, system_prompt)
        print(f"\n🐛 [DEBUG] System Prompt for Session {session_id}:\n{messages[0]['content'][:300]}...\n")
    except Exception as e:
        print(f"[WARN] Redis context get_context failed: {e}")
    ...
    if stream:
        ...
        def stream_wrapper():
            full_response = ""
            stream_gen = chat_stream(messages=messages, max_tokens=256, temperature=0.7)
